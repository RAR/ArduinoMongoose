...
bool MongooseMqttClient::subscribe(const char *topic)
{
  if(connected())
  {
    ...
    // With a constant id, brokers that dedupe in-flight packet ids by treating
    // repeats as retransmissions (AWS IoT Core does) silently drop every
    // subscription after the first when several SUBSCRIBEs race their SUBACKs.
